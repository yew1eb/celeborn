...
package org.apache.celeborn.trogdor.chaos.action
...
import org.apache.celeborn.trogdor.chaos.plan.exception.PlanInvalidException
import org.apache.celeborn.trogdor.chaos.runner.RunnerContext
...
abstract class DiskAction(
  ...
  override def generateOperations(context: SchedulerContext): List[Operation] = {
    ...
    targets.map(ta => {
      ...
      val badInflightFile = context.conf.planActionBadInflightFile
...
abstract class NodeAction(
  ...
  private val startMasterScript: String = conf.startMasterScript
  private val stopMasterScript: String = conf.stopMasterScript
  private val startWorkerScript: String = conf.startWorkerScript
  private val stopWorkerScript: String = conf.stopWorkerScript
...
object Action {
  ...
  def parseJson(obj: JSONObject, conf: ChaosConf): Action = {
    ...
    val interval =
      if (selectorObj.containsKey("interval")) {
        ...
        Utils.timeStringAsMs(conf.planActionDefaultInterval)
      }
    idOpt.get match {
      ...
      case ActionIdentity.ACTION_OCCUPY_CPU =>
        new OccupyCpuAction(
          ...
          Math.min(
            ...
            conf.planActionOccupyCpuMaxDurationMs),
          interval)
