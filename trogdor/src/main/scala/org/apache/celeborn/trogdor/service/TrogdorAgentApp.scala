...
import org.apache.celeborn.trogdor.platform.{BasicPlatform, Platform, PlatformConfig}

object TrogdorAgentApp extends Logging {
  def main(args: Array[String]): Unit = {
    ...
    val agent = new TrogdorAgent(conf, platform)
    try {
      agent.initialize()
      logInfo(s"TrogdorAgent started on ${agent.connectionUrl}")
      ShutdownHookManager.get().addShutdownHook(
        new Runnable {
          override def run(): Unit = {
            logInfo("Shutting down TrogdorAgent.")
            agent.stop(0)
          }
        },
        0)
      ...
      case e: Throwable =>
        logError("Failed to start TrogdorAgent.", e)
        agent.stop(1)
        System.exit(1)
    }
  }
