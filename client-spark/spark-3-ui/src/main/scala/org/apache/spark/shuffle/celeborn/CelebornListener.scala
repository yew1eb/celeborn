 ...
 *  APP_STATUS_QUEUE (single consumer thread, so the mutable accumulators below need no
 *  external synchronization) so events flow into the event log for HistoryServer replay.
 *
 ...
 */
private[celeborn] class CelebornListener(val kvstore: KVStore, val conf: SparkConf)
  ...
  // onFlush / addTrigger only exist on ElementTrackingStore. Both the live status store and
  // the HistoryServer replay store are ElementTrackingStore in practice; anything else skips
  // the final-flush hook and the retention trigger.
  kvstore match {
    case trackingStore: ElementTrackingStore =>
      ...
      // Without this, the trailing TaskMetrics would be lost from both the live store rebuild
      // and the HistoryServer replay.
      trackingStore.onFlush {
        mayUpdate(true)
      }
      trackingStore.addTrigger(classOf[CelebornShuffleAssignmentUIData], retainedShuffles.toLong) {
        count => cleanupAssignments(count)
      }
    case _ =>
      logWarning(
        s"KVStore ${kvstore.getClass.getName} is not an ElementTrackingStore, " +
          "Celeborn UI final flush and retention trigger are disabled.")
  }

  /** Register to the status queue so its events enter the event log. */
  def register(sc: SparkContext): Unit = {
    sc.listenerBus.addToStatusQueue(this)
  }
